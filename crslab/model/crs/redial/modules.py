...
class HRNN(nn.Module):
    ...
    def get_utterance_encoding(self, context, utterance_lengths):
        ...
        # reshape and reorder
        sorted_utterances = context.view(
        ...
        packed_utterances = pack_padded_sequence(
        ...
        utterance_encoding = torch.cat(
            (utterance_encoding[-1], utterance_encoding[-2]), 1)
        if self.use_dropout:
    ...
    def forward(self, context, utterance_lengths, dialog_lengths):
        ...
        """
        utterance_encoding = self.get_utterance_encoding(
            context, utterance_lengths)  # (bs, conv_len, 2 * utt_dim)
        sorted_lengths, sorted_idx, rev_idx = sort_for_packed_sequence(
            dialog_lengths)
        ...
        packed_sequences = pack_padded_sequence(
            sorted_representations, sorted_lengths.cpu(), batch_first=True)
...
class SwitchingDecoder(nn.Module):
    ...
    def forward(self, request, request_lengths, context_state):
        ...
        # (batch_size, max_utterance_length, embed_dim)
        embedded_request = self.embedding(sorted_request)
        packed_request = pack_padded_sequence(
            embedded_request, sorted_lengths.cpu(), batch_first=True)
        ...
        # (batch_size, max_utterance_length, vocab_size)
        sorted_language_output = self.out(sorted_vocab_state)
